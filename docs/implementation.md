...
2. ゴール引換 API（4桁コードの発行と消込）と、スタッフが押す受取完了
3. `vite-plugin-pwa` による静的リソースの事前キャッシュと、画像の最適化
4. 食品タブ、特典タブ、イラストのキャンパスマップ
...
- `startHomeAutoFlow`（800ms の自動進行）は本番の進行から外す。開発用に残すなら、本番ビルドでは無効にする。
...
| ゴール API（`/api/token/*`） | Network First。オフライン時は発行・消込できない旨を画面に出す |
