...
届け方は次の順にする。プレリリース1の器は、いまのトップ・囁き・ゴール画面である。地図の見た目はプレリリース2で合意する。

| 段階 | クライアントで足すもの | 企画と凍結すること |
|------|------------------------|--------------------|
| Phase 1 | `localStorage`、URL スタンプ、自動進行の削除 | — |
| Phase 2 | 発行・消込・上限終了・再表示 | プレリリース1。状態の進み方と受付操作 |
| Phase 3 | ダミー SVG のマップ、ボトムシート、明治レトロの配色と部品 | — |
| Phase 4 | PWA、食品タブ、プレゼントタブ | プレリリース2。操作感、導線、オフライン。SVG の座標系 |
| Final | 本番 SVG への差し替え、ピン位置の最終調整 | 印刷用 QR は実機読み取りのあと |

プレリリース1では「画面の器は次のデモで地図に変わる。今回決めるのは、どのスタンプを持っているかと、受付の操作」と伝える。配色の切り替えも Phase 3 からにする。プレリリース1はいまのダーク紫のまま、導線と受付だけを見る。
